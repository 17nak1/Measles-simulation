...
pfilter.run = function(n, input){

  input = input[0];
  let defaults = { parameters:-1, Np:-1, toler:1e-17, maxFail:Infinity, runPredMean:0, runPredVar:0, runFilterMean:0,
    runSaveStates:0, timeZero:-1, dt:-1, dataCases:0 }
  ...
  if (input.parameters === -1 || input.Np === -1 || input.timeZero === -1 || input.dt === -1) {
  ...
  let doPredictionVariance = 0, doPredictionMean = 1, runFilterMean = 0 , allFail = 0
  ...
  state = snippet.initz(interpolPop(t0), params);
  ...
  for (k = t0; k < tdata; k += deltaT){
    ...
    particles = simulator (Np, temp, dt, interpolPop, interpolBirth, params, k, k2)
  ...
  for (let timeCountData = 0; timeCountData < timeLen; timeCountData++){
    if (typeof progress === 'function') progress(timeCountData/ timeLen);
    ...
    for (np = 0; np < Np; np++){ 
      ...
      likvalue = snippet.dmeasure(params.rho, params.psi, particles[np][4], modelCases, giveLog = 0)
  ...
  let infected = []
  for (let i = 0; i < predictionMean.length; i++) {
    infected.push(predictionMean[i][4]);
  }
